...
void SimulateBuild(FlowLine &FL, int &i, int &j)
{
	...
	else //don't skip
	{
		if (CheckIfFinished(FL.TheWorkArea[i].Stations[j]->TimeLeft) == true)
		{
			...
			else //The Build exception. Unit can move out of build line if complete regardless of if it's at the end of the line
			{
